...
import { database, databaseAdmin } from '../lib/database.js';
...
router.get('/', authenticateDatabase, async (req, res) => {
  try {
...
router.get('/:id', authenticateDatabase, async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/', authenticateDatabase, async (req, res) => {
  try {
...
router.put('/:id', authenticateDatabase, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, content, description, modality, body_part, tags, is_default } = req.body;
    
    // Check if template exists and belongs to user
    const { data: existingTemplate, error: fetchError } = await database
      .from('templates')
      .select('id')
      .eq('id', id)
      .eq('user_id', req.user.id)
...
router.delete('/:id', authenticateDatabase, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/default', authenticateDatabase, async (req, res) => {
  try {
    const { data: template, error } = await database
      .from('templates')
      .select('*')
...
router.post('/:id/set-default', authenticateDatabase, async (req, res) => {
  try {
    const { id } = req.params;
