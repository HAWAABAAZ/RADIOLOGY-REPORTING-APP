...
router.get('/all', authenticateDatabase, async (req, res) => {
  try {
...
router.patch('/:id', authenticateDatabase, async (req, res) => {
  try {
    // Check if user is admin (DB row or allowlisted email)
    let isAdmin = false;
    if (isEmailAllowlisted(req.user.email)) {
...
router.get('/stats', authenticateDatabase, async (req, res) => {
  try {
    // Check if user is admin (DB row or allowlisted email)
    let isAdmin = false;
    if (isEmailAllowlisted(req.user.email)) {
    ...
    // Get overall stats
    const { data: totalStats, error: totalError } = await database
      .from('feedback')
