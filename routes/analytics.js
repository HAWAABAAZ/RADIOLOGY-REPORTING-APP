...
// Track user activity
router.post('/track', authenticateDatabase, async (req, res) => {
  try {
...
// Manage user session
router.post('/session', authenticateDatabase, async (req, res) => {
  try {
...
router.get('/dashboard', authenticateDatabase, async (req, res) => {
  try {
    ...
    const { data: dailyStats, error: dailyError } = await database
    ...
    const { data: activeSessions, error: sessionError } = await database
      .from('user_sessions')
    ...
    const { data: recentActivities, error: activityError } = await database
...
router.get('/live-users', authenticateDatabase, async (req, res) => {
  try {
    // Check if user is admin (DB row or allowlisted email)
    let isAdmin = false;
    if (isEmailAllowlisted(req.user.email)) {
    ...
    const { data: activeSessions, error } = await database
      .from('user_sessions')
      .select(`
    ...
    if (userIds.length > 0) {
      const { data: users, error: userError } = await database.auth.admin.listUsers();
...
router.post('/update-daily-stats', authenticateDatabase, async (req, res) => {
  try {
    // Check if user is admin (DB row or allowlisted email)
    let isAdmin = false;
    if (isEmailAllowlisted(req.user.email)) {
    ...
    const { error } = await database.rpc('update_daily_stats');
...
router.get('/user-activities', authenticateDatabase, async (req, res) => {
  try {
    // Check if user is admin (DB row or allowlisted email)
    let isAdmin = false;
    if (isEmailAllowlisted(req.user.email)) {
