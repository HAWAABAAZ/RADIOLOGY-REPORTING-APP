...
router.post('/register', async (req, res) => {
  try {
    ...
    if (authError) {
      console.error('Database registration error:', authError);
      throw authError;
    }
    ...
    if (authData.user) {
      ...
      const { error: profileError } = await databaseAdmin
        .from('user_profiles')
        .insert([
          {
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Get user profile
    let { data: profile, error: profileError } = await database
      .from('user_profiles')
      .select('*')
    ...
    if (profileError && profileError.code === 'PGRST116') {
      ...
      const { data: newProfile, error: createError } = await databaseAdmin
        .from('user_profiles')
        .insert([
          {
